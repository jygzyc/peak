...
peak start ./my-board           # 创建/附加 Project 并运行 Plan/Supervise/Execute
peak serve                      # 只提供持久化 Graph API + Web UI，不启动 Worker
```

`peak start` 会在后台启动 Runtime 与 HTTP Server，打印 PID、Web URL 和日志路径后返回。使用 `peak status` 查看状态，使用 `peak stop [task-name]` 优雅关闭。`peak resume <project-uuid> [board]` 按 UUID 附加一个持久化 Project 并校验其 Goal。
...
- 每个 Project 恰好为 `{id?, source, goal}`；`source` 直接成为不可变 `origin` Fact 描述。Project `id` 初始为空；首次 `start` 会把生成的 UUID 原子写回 `task.json`。非空 id（UUID）表示附加并复用持久化 Graph。
- 每个 `workers[]` 配置项由 Worker 定义 `{type, model?, env}` 和仅属于配置层的路由元数据 `{taskTypes, maxRunning, priority}` 组成。`taskTypes` 只供 Runtime 路由使用，不会传入 Worker 模块。`env` 携带 Worker 级环境变量（如 `ANTHROPIC_API_KEY`、`PI_MODEL`）并合入 CLI 子进程；没有自由形式的 `args` 字段。空 `model` 表示使用 Agent 工具默认模型。
...
peak start [board-directory]         在后台启动 Projects
...
后台输出写入 `<peak-home>/server.log`；本地进程元数据仅供 `status` 和 `stop` 使用。常用选项：`--host`、`--port`（`0` = 临时端口）、`--peak-home`、`--no-install-skills`。Peak 的 Graph API 对外公开，不内置访问 token 层。完成时 `start` 会为物化到 Project shard `out/` 目录（`~/.peak/projects/<uuid>/out/`）的每个最终交付物打印 `[peak] deliverable: <path>`。

`export` 只接受 completed Project。归档包含 `manifest.json`（内含可直接加入 `board.projects` 的 JSON 区块）、`graph.json`、一致性的 `project.db` 快照和全部已注册的内容寻址 Artifact。`import` 会校验数据库、Graph JSON、Artifact 元数据/大小/SHA-256，以原 UUID 恢复且绝不覆盖已有 Project；随后把命令输出的 JSON 区块放入目标 Board 的 `task.json` 即可复用。
