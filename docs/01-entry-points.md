...
### 用途
`decx-agent` 包的 **公共 API 出口（barrel）**。不包含任何运行时逻辑，仅做 `export` 聚合，把散落在各子模块的对外类型/函数统一对外暴露。
...
- ⚠️ barrel 文件天然削弱 tree-shaking；若后续要做 SDK 子路径导出（如 `decx-agent/graph`），当前 `package.json` 未做 `exports` 子路径映射。
...
### 用途
`decx-agent` 命令行入口。基于 `commander` 构建命令树，解析参数后把运行时构造委托给 `app/agent-runtime.ts`。自身刻意保持 thin。
...
| `sessions` | 列出所有分析会话 | `--base-dir`(默认 `.decx-analysis`) |
...
`resume` 的执行链：`SessionManager(".decx-analysis")` → `open(session)` → 取 `projects[0]` → 置 `active` → `new SessionLoop` → 可选 `HttpServer` → `loop.run()`。
...
- ⚠️ **baseDir 处理不一致**：`run`/`resume`/`status` 硬编码 `SessionManager(".decx-analysis")`，`sessions`/`search` 提供 `--base-dir`。路径相对 `process.cwd()`，跨目录行为不可预期。
...
- `cli.ts` 直接 `new SessionManager(".decx-analysis")`，而 `DECX_HOME` 重定向机制在 `decx-cli` 而非本包；本包的 baseDir 完全靠 CLI 字面量，与 AGENTS.md「`.decx/agent_tasks/`」默认路径也不一致（这里用的是 `.decx-analysis/`）。**两套路径约定并存，易混淆**。
...
1. **入口三处版本/路径硬编码**：`cli.ts` 的 `0.1.0`、`.decx-analysis`、`25429` 端口；建议集中到 `app/version.ts` + 配置。
