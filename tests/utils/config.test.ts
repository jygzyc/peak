...
test("config is strict and initializes Board skills", () => {
  ...
  try {
    ...
    const promptTask = {
      board: { skills: ["review", "research"], projects: [{ source: "Main", goal: "done" }] },
      workers: [{ type: "pi", taskTypes: ["supervise", "execute"] }],
      phase: {
        plan: { customProfile: { description: "Use for security planning.", prompt: "Plan every proof edge.", skills: ["review"] } },
        supervise: { customProfile: { description: "Use for proof review.", prompt: "Check every proof edge.", skills: ["review"] } },
        execute: { maxArtifactBytes: 2_048, customProfile: [{ description: "Use for primary research.", prompt: "Collect primary evidence.", skills: ["research"] }] },
      },
    };
    ...
    assert.deepEqual(promptConfig.phase.supervise, {
      intervalMs: 60_000,
      customProfile: { description: "Use for proof review.", prompt: "Check every proof edge.", skills: ["review"] },
    });
    assert.deepEqual(promptConfig.phase.execute.customProfile, [
      { description: "Use for primary research.", prompt: "Collect primary evidence.", skills: ["research"] },
    ]);
    ...
    (promptTask.phase.supervise.customProfile as { prompt?: string; skills?: string[] }).prompt = "Check every proof edge.";
    (promptTask.phase.supervise.customProfile as { skills?: string[] }).skills = ["missing"];
    writeFileSync(join(promptsDir, "task.json"), JSON.stringify(promptTask));
    assert.throws(() => loadTaskConfig(promptsDir), /references undeclared board Skill: missing/);

    (promptTask.phase.supervise.customProfile as { skills?: string[] }).skills = ["review", "review"];
    writeFileSync(join(promptsDir, "task.json"), JSON.stringify(promptTask));
    assert.throws(() => loadTaskConfig(promptsDir), /skills contains duplicates/);
  } finally { rmSync(root, { recursive: true, force: true }); }
});
...
test("config parses worker env and rejects removed scheduler/args fields", () => {
  ...
  try {
    ...
    writeFileSync(join(root, "task.json"), JSON.stringify({
      ...
      workers: [
        { type: "pi", taskTypes: ["plan", "supervise"], env: { PI_MODEL: "deepseek/v4", TOOL_SETTING: "value" } },
        { type: "codex", taskTypes: ["execute"], maxRunning: 2, env: { CODEX_MODEL: "gpt-5" } },
      ],
    }));
    const config = loadTaskConfig(root);
    assert.deepEqual(config.workers["worker-1"]?.env, { PI_MODEL: "deepseek/v4", TOOL_SETTING: "value" });
...
test("config parses task execution and rejects removed container knobs", () => {
  const root = mkdtempSync(join(tmpdir(), "peak-execution-"));
  try {
    ...
    const config = loadTaskConfig(taskDir);
    assert.deepEqual(config.execution, { mode: "docker", networkMode: "host" });
    assert.equal(Object.isFrozen(config.execution), true);

    const containerDir = join(root, "container");
    mkdirSync(containerDir);
    writeFileSync(join(containerDir, "task.json"), JSON.stringify({
      board: { projects: [{ source: "Main", goal: "done" }] },
      workers: [{ type: "pi", taskTypes: ["supervise", "execute"] }],
      container: { networkMode: "host" },
    }));
    assert.throws(() => loadTaskConfig(containerDir), /task config contains unknown field "container"/);

    const extraDir = join(root, "extra");
    mkdirSync(extraDir);
    writeFileSync(join(extraDir, "task.json"), JSON.stringify({
      board: { projects: [{ source: "Main", goal: "done" }] },
      workers: [{ type: "pi", taskTypes: ["supervise", "execute"] }],
      execution: { mode: "docker", capAdd: ["NET_RAW"] },
    }));
    assert.throws(() => loadTaskConfig(extraDir), /execution contains unknown field "capAdd"/);

    const oldBoardDir = join(root, "old-board-mode");
    mkdirSync(oldBoardDir);
    writeFileSync(join(oldBoardDir, "task.json"), JSON.stringify({
      board: { mode: "docker", projects: [{ source: "Main", goal: "done" }] },
      workers: [{ type: "pi", taskTypes: ["supervise", "execute"] }],
    }));
    assert.throws(() => loadTaskConfig(oldBoardDir), /board contains unknown field "mode"/);

    const badModeDir = join(root, "bad-mode");
    mkdirSync(badModeDir);
    writeFileSync(join(badModeDir, "task.json"), JSON.stringify({
      board: { projects: [{ source: "Main", goal: "done" }] },
      workers: [{ type: "pi", taskTypes: ["supervise", "execute"] }],
      execution: { mode: "container" },
    }));
    assert.throws(() => loadTaskConfig(badModeDir), /execution\.mode must be one of: local, docker/);

    const missingModeDir = join(root, "missing-mode");
    mkdirSync(missingModeDir);
    writeFileSync(join(missingModeDir, "task.json"), JSON.stringify({
      board: { projects: [{ source: "Main", goal: "done" }] },
      workers: [{ type: "pi", taskTypes: ["supervise", "execute"] }],
      execution: { networkMode: "host" },
    }));
    assert.throws(() => loadTaskConfig(missingModeDir), /execution\.mode is required/);

    const noneDir = join(root, "none");
    mkdirSync(noneDir);
    writeFileSync(join(noneDir, "task.json"), JSON.stringify({
      board: { projects: [{ source: "Main", goal: "done" }] },
      workers: [{ type: "pi", taskTypes: ["supervise", "execute"] }],
    }));
    assert.deepEqual(loadTaskConfig(noneDir).execution, { mode: "local" });
  } finally { rmSync(root, { recursive: true, force: true }); }
});
