...
test("task management API validates, lists, starts, stops, and deletes tasks", async () => {
  ...
  const server = new GraphHttpServer(registry, undefined, [projectRegistrationExtension(peakHome), taskManagerExtension(context)]);
  ...
  try {
    ...
    // Start (local): the server spawns the independent `peak dispatch --graph-url` role.
    const started = await api(server.baseUrl, "/api/tasks/managed/start", "POST", {});
    ...
    const again = await api(server.baseUrl, "/api/tasks/managed/start", "POST", {});
    ...
    await api(server.baseUrl, "/api/tasks/purged/start", "POST", {});
