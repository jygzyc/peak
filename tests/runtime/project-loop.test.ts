...
test("ProjectLoop records phase_failed events in the Project log", async () => {
  ...
  try {
    ...
    const loop = new ProjectLoop(project.id, config(), graph, executor as never, executions, () => "[]");
    await loop.tick();
...
function config(): ResolvedTaskConfig {
  return {
    ...
    board: { port: 40_000, skills: [], projects: [{ source: "start", goal: "done" }] },
...
test("ProjectLoop Execute budgets are independent across Projects", async () => {
  ...
  try {
    ...
    const executions = new ExecutionRegistry();
    const executor = new RecordingExecutor();
    const loopA = new ProjectLoop(first.id, config(), graph, executor as never, executions, () => "[]");
    const loopB = new ProjectLoop(second.id, config(), graph, executor as never, executions, () => "[]");
...
test("ProjectLoop cleans up the per-Project runtime scratch directory once the Project is no longer active", async () => {
  ...
  try {
    const project = await graph.createProject({ title: "P", target: "open", goal: "done", scope: "s" });
    const executor = new RecordingExecutor();
    const loop = new ProjectLoop(project.id, config(), graph, executor as never, new ExecutionRegistry(), () => "[]");
...
test("ProjectLoop releases and re-establishes the execution target exactly once per status transition", async () => {
  const projects = mkdtempSync(join(tmpdir(), "peak-loop-"));
  mkdirSync(projects, { recursive: true });
  const registry = new ProjectStoreRegistry(projects);
  const server = new GraphHttpServer(registry);
  ...
  try {
    const project = await graph.createProject({ title: "P", target: "open", goal: "done", scope: "s" });
    const executor = new RecordingExecutor();
    const hooks = { inactive: 0, activated: 0, statuses: [] as string[] };
    const loop = new ProjectLoop(project.id, config(), graph, executor as never, new ExecutionRegistry(), () => "[]", {
      onInactive: (status) => { hooks.inactive += 1; hooks.statuses.push(status); },
      onActivated: async () => { hooks.activated += 1; },
    });
    // Completed/stopped before the first tick: the target is released on the
    // very first observation of a non-active status.
    await graph.setStatus(project.id, "stopped");
    assert.equal(await loop.tick(), 0);
    assert.equal(hooks.inactive, 1, "target released once on entering non-active");
    assert.deepEqual(hooks.statuses, ["stopped"], "the reason status reaches the release hook");
    assert.equal(executor.cleanupCalls, 1);
    await loop.tick();
    assert.equal(hooks.inactive, 1, "no repeat release while staying non-active");
    assert.equal(executor.cleanupCalls, 1);
    // Re-activation re-establishes the target before scheduling resumes.
    await graph.setStatus(project.id, "active");
    await loop.tick();
    assert.equal(hooks.activated, 1, "target re-established on re-activation");
    assert.equal(hooks.inactive, 1);
    // And the next inactive transition releases it again.
    await graph.setStatus(project.id, "stopped");
    await loop.tick();
    assert.equal(hooks.inactive, 2, "target released again on the second inactive transition");
    assert.deepEqual(hooks.statuses, ["stopped", "stopped"]);
