...
test("AgentRuntime attaches only prepared Projects through an external Graph URL", async () => {
  ...
  try {
    ...
    const runtime = new AgentRuntime(
      ...
      { graphUrl: server.baseUrl, projectsRoot: join(root, "runtime-projects"), installSkills: false },
    );
    ...
    // Dispatch never creates: a configured Project without an id is rejected.
    const creating = new AgentRuntime(
      configuration(root, "start", "done"),
      { graphUrl: server.baseUrl, projectsRoot: join(root, "runtime-projects"), installSkills: false },
    );
    await assert.rejects(creating.start(), /requires prepared Project ids/);
  } finally {
    await server.stop();
    registry.close();
