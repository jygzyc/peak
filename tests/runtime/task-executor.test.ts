...
import { customProfileDigest, type ResolvedTaskConfig, type TaskType } from "../../dist/utils/types.js";
...
import { placedSourceName } from "../../dist/runtime/execution-backend.js";
...
class TestJointPlan {
  ...
  private readonly values: JointPlanPath[] = [];
  register(projectId: string): void { this.projectIds.add(projectId); }
  addPath(path: JointPlanPath): void { this.values.push(path); }
  paths(targetProjectId: string): JointPlanPath[] {
    return this.values.filter((path) => path.projectId !== targetProjectId && this.projectIds.has(path.projectId));
  }
}
...
test("Plan, Supervise and Execute mutate Graph only through HTTP", async () => {
  ...
  try {
    ...
    const federation = new TestJointPlan();
    ...
    // Execute does not analyze: Joint Plan generates the incremental summary
    // immediately before the next Plan Worker call.
    ...
    const archiveManifest = await registry.exportProjectArchive(project.id, join(root, "completed.tar.gz"));
    assert.deepEqual(archiveManifest.pathAbstracts.map((item) => item.factId), ["f0001"],
      "a Project completed by Plan exports its full leaf PathAbstract frontier without export-time Analyze");
    ...
    const executeSnapshot = JSON.parse(readFileSync(join(projectDir, "logs", executeLog), "utf8")) as {
      ...
      customProfile: { description: string; skills: string[]; digest: string };
      executionId: string;
      at: string;
    };
    ...
    assert.deepEqual(executeSnapshot.customProfile.skills, ["review"]);
    assert.equal("skills" in executeSnapshot, false, "Skills are scoped under the selected customProfile");
    assert.equal("workers" in executeSnapshot, false, "Worker configuration is not persisted in phase snapshots");
    ...
    const plans = planLogs.map((name) => JSON.parse(readFileSync(join(projectDir, "logs", name), "utf8")) as {
      ...
      context: {
        ...
        external: Array<{ factRef: { projectId: string; id: string; description: string }; pathOverview: string; verifiedCore: string[] }>;
        truncated: boolean;
    ...
    assert.equal(initialPlan.context.external[0]!.pathOverview, "external path summary");
    assert.deepEqual(initialPlan.context.external[0]!.verifiedCore, ["external verified"]);
    ...
    assert.doesNotMatch(workers.calls.find((call) => call.prompt.startsWith("# Analyze"))!.prompt, /Available Skills|"review"/);
    ...
    assert.match(supervisePrompt, /Available Skills:[\s\S]*"review"/);
...
test("Execute resumes a failed started worker through Finalize", async () => {
  ...
  try {
    const project = await graph.createProject({ title: "P", target: "start", goal: "done", scope: "s" });
    const config = configuration(root);
    const executeProfile = config.phase.execute.customProfile[0]!;
    const intent = await graph.createIntent(project.id, {
      from: [{ projectId: project.id, id: "origin", description: "start" }],
      customProfile: executeProfile.description,
      customProfileDigest: customProfileDigest(executeProfile),
...
/** Workspace that places source Artifacts under its own sources dir (like LocalBackend). */
function placingWorkspace(tmpDir: string) {
  const sourcesDir = join(tmpDir, "sources");
  mkdirSync(sourcesDir, { recursive: true });
  return {
    tmpDir,
    cleanup: (): void => undefined,
    placeArtifact: async (artifact: { sha256: string; filename: string | null }, content: Buffer) => {
      const inputPath = join(sourcesDir, placedSourceName(artifact));
      writeFileSync(inputPath, content);
      return { inputPath, read: async () => readFileSync(inputPath) };
    },
  };
}

test("Execute materializes source Artifacts into the workspace and detects tampering there", async () => {
  const root = mkdtempSync(join(tmpdir(), "peak-source-placed-"));
  const projects = join(root, "projects");
  mkdirSync(projects, { recursive: true });
  const registry = new ProjectStoreRegistry(projects);
  const server = new GraphHttpServer(registry);
  ...
  try {
    ...
    const federation = new TestJointPlan();
    federation.register(project.id, join(projects, project.id));
    const workspace = placingWorkspace(join(root, "ws"));
    const workers = new SourceMutatingWorkers();
    const executor = new TaskExecutor(
      configuration(root),
      { key: "project-1", id: project.id, source: "immutable source", goal: "done" },
      graph,
    ...
    await assert.rejects(executor.execute(project.id, intent, "e-placed"), /Artifact size or type mismatch|Artifact hash mismatch/);
    // The worker received the workspace copy (it mutated it), and the
    // post-run integrity check caught the tampered bytes via read().
    const placed = readdirSync(join(workspace.tmpDir, "sources"));
    assert.equal(placed.length, 1, "the source Artifact was placed into the workspace");
    assert.notEqual(readFileSync(join(workspace.tmpDir, "sources", placed[0]!), "utf8"), "immutable source details\n",
      "the worker mutated the placed copy");
    const result = await graph.getProject(project.id);
    assert.equal(result.intents[1]?.to, null, "tampered Execute never concludes");
  } finally {
    await server.stop();
    registry.close();
    rmSync(root, { recursive: true, force: true });
...
test("Execute fetches the source Artifact over HTTP when no local body exists (remote Dispatch)", async () => {
  const root = mkdtempSync(join(tmpdir(), "peak-source-remote-"));
  const projects = join(root, "projects");
  mkdirSync(projects, { recursive: true });
  const registry = new ProjectStoreRegistry(projects);
  const server = new GraphHttpServer(registry);
  ...
  const graph = new GraphClient(server.baseUrl, { projectsRoot: projects });
  try {
    const project = await graph.createProject({ title: "P", target: "remote source", goal: "done" });
    const sourcePath = join(root, "source.md");
    writeFileSync(sourcePath, "remote immutable source details\n");
    const sourceArtifact = await graph.uploadArtifact(project.id, sourcePath, "text/markdown");
    const sourceIntent = await graph.createIntent(project.id, {
      from: [{ projectId: project.id, id: "origin", description: "remote source" }],
      description: "Materialize the source", createdBy: "test",
    });
    const source = await graph.conclude(project.id, sourceIntent.id, {
      description: "Remote source file", artifact: sourceArtifact, concludedBy: "test",
    });
    const intent = await graph.createIntent(project.id, {
      from: [{ projectId: project.id, id: source.fact.id, description: source.fact.description }],
      description: "Read the remote source", createdBy: "test",
    });
    // Simulate a remote Dispatch: its GraphClient anchors Artifact paths under
    // a different Projects root, so no local body exists and the Runtime must
    // fetch the canonical bytes from the Server over HTTP.
    const remoteGraph = new GraphClient(server.baseUrl, { projectsRoot: join(root, "dispatch-projects") });
    const federation = new TestJointPlan();
    federation.register(project.id, join(projects, project.id));
    const workspace = placingWorkspace(join(root, "ws"));
    const workers = new FakeWorkers();
    workers.outputs.execute.push('{"kind":"fact","description":"read and verified","artifact":null}');
    const executor = new TaskExecutor(
      configuration(root),
      { key: "project-1", id: project.id, source: "remote source", goal: "done" },
      remoteGraph,
      workers,
      federation,
      join(projects, project.id),
    ...
    await executor.execute(project.id, intent, "e-remote");

    const call = workers.calls.find((item) => item.type === "execute");
    assert.ok(call, "the Execute worker was invoked");
    const placed = readdirSync(join(workspace.tmpDir, "sources"));
    assert.equal(placed.length, 1);
    // The prompt carries the JSON-escaped workspace path (backslashes are
    // doubled inside the serialized Graph view).
    assert.ok(call!.prompt.includes(JSON.stringify(join(workspace.tmpDir, "sources", placed[0]!)).slice(1, -1)),
      "the prompt exposes the workspace copy path");
    assert.equal(readFileSync(join(workspace.tmpDir, "sources", placed[0]!), "utf8"), "remote immutable source details\n",
      "the HTTP-fetched bytes were placed for the worker");
    const result = await graph.getProject(project.id);
    assert.ok(result.intents[1]!.to, "the Execute concluded with the remotely fetched source");
  } finally {
    await server.stop();
    registry.close();
    rmSync(root, { recursive: true, force: true });
...
test("prepareJointPlan caches deterministic Path Abstracts and never re-runs the worker", async () => {
  ...
  try {
    ...
    await executor.prepareJointPlan(project.id);
...
test("prepareJointPlan falls back to a description join when the worker keeps failing", async () => {
...
test("prepareJointPlan persists structured Path Abstract content", async () => {
...
class StaleInjectingWorkers implements TaskWorkers {
  ...
  async execute(_name: string, type: TaskType, prompt: string): Promise<WorkerResult> {
    ...
    if (prompt.startsWith("# Analyze")) {
      const output = JSON.stringify({ pathOverview: "incremental concurrent path", verifiedCore: [this.f002] });
...
test("Plan re-plans instead of failing when a concurrent Execute consumes a source leaf", async () => {
  ...
  try {
    ...
    await seedExecutor.prepareJointPlan(project.id);
...
test("every worker round-trip is audited in main.log: started, completed, timeout, failed", async () => {
  const root = mkdtempSync(join(tmpdir(), "peak-audit-"));
  const projects = join(root, "projects");
  mkdirSync(projects, { recursive: true });
  const registry = new ProjectStoreRegistry(projects);
  const server = new GraphHttpServer(registry);
  ...
  try {
    ...
    const executor = new TaskExecutor(
      config,
      { key: "project-1", source: "start", goal: "done" },
      graph, workers, new TestJointPlan(), projectDir,
    );

    // Clean success.
    workers.outputs.supervise.push('{"kind":"hint","content":"verify"}');
    await executor.supervise(project.id, "s-ok");
    // One timed-out attempt (retryable) followed by a clean attempt.
    workers.outputs.supervise.push({ text: "", stdout: "", stderr: "killed", returncode: 1, timedOut: true, cancelled: false, started: true });
    workers.outputs.supervise.push('{"kind":"hint","content":"verify after retry"}');
    await executor.supervise(project.id, "s-timeout");
    // Non-zero exits are retried until the attempt budget is exhausted.
    workers.outputs.supervise.push(
      { text: "", stdout: "", stderr: "boom", returncode: 2, timedOut: false, cancelled: false, started: true },
      { text: "", stdout: "", stderr: "boom", returncode: 2, timedOut: false, cancelled: false, started: true },
      { text: "", stdout: "", stderr: "boom", returncode: 2, timedOut: false, cancelled: false, started: true },
    );
    await assert.rejects(executor.supervise(project.id, "s-fail"), /supervise worker failed/);
    // One Execute round-trip: its worker events must pinpoint the Intent.
    const intent = await graph.createIntent(project.id, {
      from: [{ projectId: project.id, id: "origin", description: "start" }],
      description: "do the execute step", createdBy: "test",
    });
    workers.outputs.execute.push('{"kind":"fact","description":"step done","artifact":null}');
    await executor.execute(project.id, intent, "e-audit");

    const events = readFileSync(join(projectDir, "logs", "main.log"), "utf8")
      .trim().split("\n").map((line) => JSON.parse(line) as Record<string, unknown>);
    const workerEvents = events.filter((event) => String(event.type).startsWith("worker_"));
    assert.equal(workerEvents.filter((event) => event.type === "worker_started").length, 7, "one worker_started per attempt (6 supervise + 1 execute)");
    assert.equal(workerEvents.filter((event) => event.type === "worker_completed").length, 3);
    assert.equal(workerEvents.filter((event) => event.type === "worker_timeout").length, 1, "the phase deadline kill is audited as a timeout");
    const failed = workerEvents.filter((event) => event.type === "worker_failed");
    assert.equal(failed.length, 3, "every failed attempt is audited");
    for (const event of failed) {
      assert.equal(event.returncode, 2);
      assert.equal(event.stderr, "boom", "worker_failed carries a stderr preview");
    }
    const executeEvents = workerEvents.filter((event) => event.phase === "execute");
    assert.equal(executeEvents.length, 2, "one started + one terminal event for the Execute round-trip");
    for (const event of executeEvents) {
      assert.equal(event.intentId, intent.id, "Execute worker events pinpoint the open Intent");
      assert.equal(event.executionId, "e-audit");
    }
    for (const event of workerEvents) {
      assert.equal(event.projectId, project.id);
      assert.equal(event.worker, "fake");
      if (event.phase === "supervise") assert.equal(event.intentId, null, "Plan/Supervise events carry no Intent");
      if (event.type !== "worker_started") {
        assert.ok(typeof event.durationMs === "number" && (event.durationMs as number) >= 0);
        assert.ok(typeof event.returncode === "number");
      }
    }
  } finally {
    await server.stop();
...
function configuration(root: string): ResolvedTaskConfig {
  return {
    ...
    phase: {
      plan: {
        customProfile: { description: "Use for security planning.", prompt: "Plan every proof edge.", skills: ["review"] },
      },
      supervise: {
        intervalMs: 1000,
        customProfile: { description: "Use for proof review.", prompt: "Review the entire proof carefully", skills: ["review"] },
      },
      execute: {
        maxArtifactBytes: 1024,
        customProfile: [{ description: "Use for primary research.", prompt: "Collect primary evidence only", skills: ["review"] }],
      },
    },
  };
}
