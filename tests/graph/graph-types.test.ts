...
test("computePaths collects each leaf's full concluded ancestry, merges included, as one Path", () => {
  ...
  // Completion never changes a Joint Path: goal is not an Analyze node.
  graph.intents[6] = intent("7", "6", "goal");
  const completed = computePaths(graph)[0]!;
  assert.deepEqual(completed.segments[0]!.map((step) => step.fact.id), ["3", "4", "5", "6"]);
  assert.deepEqual(computePaths(graph, "goal"), []);
