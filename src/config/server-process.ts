...
export type ServerProcessMode = "run" | "resume" | "serve";
...
export function serverPidPath(peakHome: string): string { return join(peakHome, "server.pid"); }
export function serverMetadataPath(peakHome: string): string { return join(peakHome, "server.json"); }
...
export async function stopServerProcess(peakHome: string): Promise<number> {
  ...
  if (isProcessAlive(pid)) throw new Error(`Peak server did not stop within 5 seconds (pid ${pid})`);
  cleanupFiles(peakHome);
  return pid;
}
...
function readMetadata(peakHome: string): ServerProcessMetadata | undefined {
  try {
    ...
    if (value.mode !== "run" && value.mode !== "resume" && value.mode !== "serve") return undefined;
